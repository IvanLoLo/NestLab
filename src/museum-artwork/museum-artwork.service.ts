...
export class MuseumArtworkService {
    ...
    async addArtworkMuseum(museumId: string, artworkId: string): Promise<MuseumEntity> {
        const artwork: ArtworkEntity = await this.artworkRepository.findOne({where: {id: artworkId}, relations: ['museum']});
        if(!artwork) throw new BusinessLogicException("The artwork with the given id was not found", BusinessError.NOT_FOUND);
        const museum: MuseumEntity = await this.museumRepository.findOne({where: {id: museumId}, relations: ['artworks']});
        
        if(!museum) throw new BusinessLogicException("The museum with the given id was not found", BusinessError.NOT_FOUND);
        if(!museum || !artwork) return null;
    ...
    async findArtworkByMuseumIdArtworkId(museumId: string, artworkId: string): Promise<ArtworkEntity> {
        ...
        if(!museumArtwork) throw new BusinessLogicException("The artwork with the given id is not associated to the museum", BusinessError.PRECONDITION_FAILED);
    ...
    async deleteArtworkMuseum(museumId: string, artworkId: string): Promise<MuseumEntity> {
        ...
        museum.artworks = museum.artworks.filter(e => e.id !== artwork.id);
        return await this.museumRepository.save(museum);
    }
